...
# region 글로벌 변수 선언
MODEL = YOLO("best_helmet.pt")  # YOLO11s 모델
MODEL2 = YOLO("yolo11n.pt")  # YOLO11s 모델
# endregion 글로벌 변수 끝
    ...
    def process(self, img):
        ...
        if cropped_imgs:  # 사람이 감지된 경우에만 처리
            for (cropped, bbxy) in zip(cropped_imgs, cropped_bbxy):
                helmet_detected = self.fallDetect(cropped)
                x1, y1, x2, y2 = bbxy
                if helmet_detected:
                ...
                helmet_results.append(helmet_detected)
        
        # 튜플 형식으로 각 결과를 변환
        formatted_results = tuple(f'[{res}]' for res in helmet_results)
